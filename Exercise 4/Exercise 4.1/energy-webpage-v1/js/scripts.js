...
function loadPageContent(pageUrl) {
  fetch(pageUrl)
    ...
    .then(html => {
      ...
      // Remove active-page class from all sections and set it on new content
      const oldContent = contentContainer.querySelector('.page');
      if (oldContent) {
        oldContent.classList.remove('active-page');
      }
      ...
      // Re-execute scripts from the loaded page
      ...
      scripts.forEach(script => {
        if (script.src) {
          ...
          newScript.onload = () => {
            if (pageUrl.includes('television')) {
              setTimeout(() => {
                if (typeof loadCharts === 'function') {
                  loadCharts();
                }
              }, 200);
            } else if (pageUrl.includes('data')) {
              setTimeout(() => {
      ...
      // Call chart functions for pages that need them
